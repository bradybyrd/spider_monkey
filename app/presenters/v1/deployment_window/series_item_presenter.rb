...
# All Rights Reserved.
################################################################################
class V1::DeploymentWindow::SeriesItemPresenter < V1::AbstractPresenter

  # creates a local accessor based on the passed symbol
  presents :series_item
  ...
  def to_xml( options = {})
    options.merge!(root: :series_item)
  ...
  def resource_options
    ...
    { only: safe_attributes, :include => {
        :environments => { :only => [:id, :name, :deployment_policy]},
        requests: { only: [:id] },
        events: { only: [:id, :start_at, :finish_at]}
