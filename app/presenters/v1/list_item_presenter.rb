...
# All Rights Reserved.
################################################################################
class V1::ListItemPresenter < V1::AbstractPresenter
  ...
  def resource_options
    return { :only => safe_attributes, :include => {
        :list => { :only => [ :id, :name, :created_by_id, :is_text, :created_at, :updated_at, 
                              :archive_number, :archived_at ], :methods => ['required?'] }  
      }
    }
  end

  def safe_attributes
    return [:id, :value_text, :value_num, :last_modified_by_id, :archive_number, :archived_at, :created_at, :updated_at]
  end
