...
#   position: D1:E1
# status_reason:
#   name: Status Reason (Mandatory for a Status of Closed)
#   type: in-external-single-select
#   external_resource: remedy_task_status_reasons
#   position: D2:E2
# company:
...
begin
  ...
  if [params["task_name"],params["summary"],params["notes"],params["status"],params["company"],params["status_reason"]].all? {|p| p.blank?} && params["update_actual_start_date"].eql?("No") && params["update_actual_end_date"].eql?("No")
    ...
    if params["change_request_id"].blank?
      ...
      if foreign_ids.blank?
        write_to "Operation failed: There were no tickets specified or associated with the step for updating tasks"
        exit(1)
      else
        separator = ""
        filter_string = ""
        foreign_ids.each do |fid|
          fid.strip!
          filter_string = filter_string + separator + "'RootRequestID' =\"" + fid +"\""
          separator = " OR "
        end
      end
    else
      filter_string = "'RootRequestID'"+"=\""+params["change_request_id"]+"\""
    end
                            ...
                            xml.soa(:Value, "soa:type" => "xs:anyType") do |xml|
                              xml.soa(:XmlDoc) do |xml|
                                xml.fields do |xml|
                                  ...
                                  xml.field(params["status_reason"], :name => "StatusReasonSelection") unless params["status_reason"].blank?
    ...
    if completion_code.to_i == 0
      ...
      write_to("Operation completed succesfully: Updated Entries: #{count}")
