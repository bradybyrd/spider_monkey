...
#   name: Route
#   type: in-external-single-select
#   external_resource: rlm_repo_instance_routes
#   position: A3:B3
#   required: yes
# Environment:
#   name: Environment
...
#   name: Set Properties
#   type: in-external-single-select
#   external_resource: rlm_repo_instance_properties
#   position: A6:F6
#   required: no
# Deployment Log File:
...
begin
  ...
  begin
    params.each_pair do |k,v|
      ...
      if (k =~ /^#{'DE_'}/) && !v.blank? && params['Set Properties'].present? && params['Set Properties'].split(',').include?(prop_name)
        ...
        RlmUtilities.rlm_set_q_property_value(RLM_BASE_URL, RLM_USERNAME, RLM_PASSWORD, params['Instance'], 'instance property add', prop_name, v, encrypt)
      end
    end
  rescue Exception => e1
  ...
  deployment_instance_id = RlmUtilities.deploy_package_instance(RLM_BASE_URL, RLM_USERNAME, RLM_PASSWORD, params['Instance'], params['Route'], new_env_id, params['Channels'])
  ...
  if deploy_status == 'fail' ||  deploy_status == 'cancelled' || deploy_status != 'pass'
    write_to 'Operation failed: There were some problem while deploying the repo instance.'
    exit(1)
  else
