...
def execute(script_params, parent_id, offset, max_records)
  begin
    ...
    schema_name = "CHG:TemplateQueryInterface"
    qualification = "('Login ID' = $USER$) AND ('Change Request Status' = \"Enabled\")"
    field = 'Company'
    namespaces = {
                  ...
                  xml.soa(:Parameter) do |xml|
                    ...
                    xml.soa(:Value, "soa:type" => "xs:anyType") do |xml|
                      xml.soa(:XmlDoc) do |xml|
                        xml.fields do |xml|
                          xml.field("Template Name")
                          xml.field("InstanceId")
                        end
                      end
                    end
                  end
