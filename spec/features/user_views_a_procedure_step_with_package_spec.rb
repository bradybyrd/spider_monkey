...
feature 'User views a procedure step with a package' do
  include StepHelper
  ...
  context 'package is invalid' do
    scenario 'it shows the step highlighted in red', js: true do
      ...
      visit edit_procedure_path(step.parent_object)
  ...
  def edit_step(step)
    visit edit_step_in_procedure_step_path(step, procedure_id: step.parent_object.id)
  end
