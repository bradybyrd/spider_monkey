...
describe DeploymentWindow::SeriesController do
  ...
  describe 'GET index' do
    ...
    describe 'by admin user' do
      ...
      it 'assigns all deployment_window_series as @deployment_window_series ' do
        ...
        get :index, {}
        expect(assigns(:deployment_window_series)).to eq([admin_dws, coordinator_dws, deployer_dws])
      end
    ...
    context 'collection manipulations' do
      ...
      it 'searches' do
        get :index, q: '1'
        expect(assigns(:deployment_window_series)).to eq [test1]
      end

      it 'orders' do
        get :index, order: { '0' => ['name', 'DESC'] }
        expect(assigns(:deployment_window_series)).to eq [test2, test1]
      end

      it 'filters' do
        get :index, act: 'filter', filters: { state: ['state2'] }
        expect(assigns(:deployment_window_series)).to eq [test2]
      end
    end
  end
  ...
  describe 'GET new' do
    it 'assigns a new deployment_window_series as @deployment_window_series' do
      get :new
      expect(assigns(:deployment_window_series)).to be_a_new(DeploymentWindow::Series)
    end
  end
  ...
  describe 'GET edit' do
    it 'assigns the requested deployment_window_series as @deployment_window_series' do
      ...
      expect(assigns(:deployment_window_series)).to eq(deployment_window_series)
    end
  end
  ...
  describe 'POST create' do
    describe 'with valid params' do
      ...
      it 'assigns a newly created deployment_window_series as @deployment_window_series' do
        ...
        expect(assigns(:deployment_window_series)).to be_a(DeploymentWindow::Series)
        expect(assigns(:deployment_window_series)).to be_persisted
      end

      it 'redirects to the deployment windows list' do
        post :create, { deployment_window_series: valid_attributes.merge(multiparameter_attributes) }
        expect(response).to redirect_to(deployment_window_series_index_url)
      end
  ...
  describe 'PUT update' do
    ...
    describe 'with valid params' do
      it 'updates the requested deployment_window_series' do
        ...
        put :update, { id: deployment_window_series.to_param,
                       deployment_window_series: valid_attributes.merge(multiparameter_attributes) }
      end
