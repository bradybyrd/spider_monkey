...
describe InstalledComponentsController, type: :controller do
  ...
  context '#edit' do
    specify 'without servers' do
      get :edit, { id: @installed_component.id, app_id: @app.id}
  ...
  context '#update' do
    ...
    it 'changes attribute servers' do
      ...
      expect(@installed_component.server_ids).to include(@server.id)
    end
    ...
    it 'changes attribute default_server_group' do
      ...
      @installed_component.reload
      expect(@installed_component.default_server_group_id).to eq @server_group.id
    end
    ...
    it 'changes attribute server_aspect_groups' do
      ...
      @installed_component.reload
      expect(@installed_component.server_aspect_group_ids).to include(@server_aspect_group.id)
    end
    ...
    it 'changes attribute server_aspects' do
      ...
      @installed_component.reload
      expect(@installed_component.server_aspect_ids).to include(@server_aspect.id)
    end
    ...
    it 'changes property_value' do
      ...
      put :update, { id: @installed_component.id,
                     ...
                     installed_component: { default_server_group_id: @server_group.id,
                                            ...
                                            server_ids: [@server.id]}}
      property.reload
      prop_value = property.current_property_values.where('value_holder_type' => 'InstalledComponent').first.value
      expect(prop_value).to eq 'changed_value'
    end
  end

  it '#destroy' do
    expect{ delete :destroy, { id: @installed_component.id, app_id: @app.id}
          }.to change(InstalledComponent, :count).by(-1)
  end
  ...
  context '#add_remove_servers' do
    it 'returns associated servers' do
      ...
      expect(response).to render_template(layout: false)
      servers = assigns(:available_server_associations).first.first
      expect(servers.first).to eq server
      expect(servers.last).to eql(server2)
    end
    ...
    it 'returns no servers' do
      ...
      expect(response).to render_template(layout: false)
      expect(assigns(:available_server_associations)).to eq({})
    end
  end
  ...
  it '#update_servers' do
    ...
    expect(response.body).to include("#{server2.name}")
    expect(response.body).to_not include("#{server1.name}")
  end
end
