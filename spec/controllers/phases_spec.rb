...
describe PhasesController, type: :controller do
  ...
  context '#reorder' do
    specify 'without row_type' do
      ...
      @phase.reload
      expect(@phase.insertion_point).to eq 2
      expect(response).to render_template(partial: 'phases/_phase')
    end
    ...
    specify 'with row_type' do
      ...
      rt_phase.reload
      expect(rt_phase.insertion_point).to eq 5
      expect(response).to render_template(partial: 'phases/_runtime')
    end
  end
