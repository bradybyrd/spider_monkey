...
describe ScriptsController, type: :controller do
  ...
  context '#new' do
    it 'renders template new' do
      ...
      expect(response).to render_template('shared_scripts/new', layout: false)
    end

    it 'renders template detail_new without layout' do
      get :new
      expect(response).to render_template('shared_scripts/detail_new')
    end

    it 'renders template detail_new without layout' do
      xhr :get, :new
      expect(response).to render_template('shared_scripts/detail_new', layout: false)
    end
  end
  ...
  context '#edit' do
    it 'returns error and redirects' do
      ...
      expect(response).to redirect_to(automation_scripts_path)
    end

    it 'renders template edit' do
      xhr :get, :edit, id: @script.id
      expect(response).to render_template('shared_scripts/edit', layout: false)
    end

    it 'renders template edit' do
      get :edit, id: @script.id
      expect(response).to render_template('shared_scripts/detail_edit')
    end
  end
  ...
  context '#create' do
    context 'success' do
      it 'redirects to index path' do
        ...
        expect(response).to redirect_to('/environment/automation_scripts')
      end
    ...
    context 'fails' do
      ...
      it 'shows validation errors' do
        ...
        expect(response).to render_template('misc/error_messages_for', layout: false)
      end
      ...
      it 'returns paginated records' do
        ...
        @script.stub(:save).and_return(false)
        post :create, @params
        expect(assigns(:scripts)).to match_array(scripts[0..9])
        expect(assigns(:scripts)).to_not include(scripts[10])
      end
    end
  end
  ...
  context '#initialize_arguments' do
    it 'returns flash error and redirects' do
      ...
      expect(flash[:error]).to include('Unable to find script')
  ...
  context '#update_script' do
    context 'success' do
      it 'renders template update' do
        ...
        expect(response).to render_template('shared_scripts/update')
      end
  ...
  it '#render_integration_header' do
    get :render_integration_header, { script_hash: { name: 'script name1',
                                                     ...
                                                     automation_category: 'General',
                                                     unique_identifier: 'AutomationId1' }}
    expect(response).to render_template('shared_scripts/script_integration_header.html')
  end
  ...
  context '#render_automation_form' do
    ...
    it 'renders partial automation_form' do
      ...
      expect(response).to render_template(partial: 'shared_scripts/_automation_form')
    end
    ...
    specify 'ResourceAutomation' do
      ...
      expect(response).to render_template(partial: 'scripted_resources/_form')
    end
  end
  ...
  context '#execute_mapped_resource_automation' do
    ...
    specify 'with two argument values' do
      ...
      expect(response).to render_template(partial: 'shared_scripts/_execute_mapped_resource_automation')
    end
  ...
  it '#find_script_template' do
    ...
    expect(response.body).to include("#{@script.content}")
  end
  ...
  it '#find_jobs' do
    ...
    expect(response.body).to include("<option value=''>Select</option>")
  end
  ...
  it '#build_job_parameters' do
    ...
    get :build_job_parameters, { script_id: @script.id,
                                 job: create(:job_run) }
    expect(response.body).to include('current_job')
  end
  ...
  context '#update_resource_automation_parameters' do
    ...
    it 'with step present' do
      ...
      expect(response.body).to include(@arguments[0].id.to_s)
    end

    it 'with request present' do
      test_request = create(:request)
      xhr :get, :update_resource_automation_parameters, { resource_request_id: test_request.id,
  ...
  context '#get_tree_elements' do
    ...
    it 'returns script' do
      post :get_tree_elements, argument_id: @arguments[0].id
      expect(response.body).to eq @arguments[0].to_json
    end
  end
  ...
  context '#get_table_elements' do
    it 'returns blank json' do
      ...
      expect(response).to render_template('shared_scripts/get_table_elements.js')
    end
  end
