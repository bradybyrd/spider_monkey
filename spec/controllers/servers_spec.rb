...
describe ServersController, type: :controller do
  ...
  context '#update' do
    ...
    it 'changes attributes' do
      ...
      expect(assigns(:server).name).to eq 'new name'
    ...
    it 'changes all attributes' do
      ...
      @server.reload
      expect(@server.environment_ids).to include(@environment.id)
      expect(@server.server_group_ids).to include(@server_group.id)
    ...
    it 'updates name' do
      ...
      @server.reload
      expect(@server.name).to eq 'Server_changed2'
      expect(response).to redirect_to(edit_server_path(page: 0))
    end
    ...
    it "doesn`t update attributes" do
      ...
      put :update, { id: @server.id,
                     ...
                     server: { environment_ids: @environment.id } }
      @server.reload
      expect(@server.environment_ids).to_not include(@environment.id)
      expect(response).to render_template('edit')
    end
    ...
    it 'environments and server_groups by blank arrays' do
      put :update, { id: @server.id,
                     name_update: nil }
      @server.reload
      expect(@server.environment_ids).to eq []
      expect(@server.server_group_ids).to eq []
    end
    ...
    context 'server environment associations' do
      ...
      it 'should prevent removal from environments with package reference to server' do
        ...
        @server.reload
        expect(@server.environment_ids).to eq [ @environment.id ]
      end
    end
  end
end
