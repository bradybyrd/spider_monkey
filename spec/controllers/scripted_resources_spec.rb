...
describe ScriptedResourcesController, type: :controller do
  ...
  context '#edit' do
    ...
    it 'renders template edit' do
      ...
      expect(response).to render_template('scripted_resources/edit')
    end

    it 'renders template detail_edit' do
      get :edit, id: @script.id
      expect(response).to render_template('scripted_resources/detail_edit')
    end
  end
    ...
    context 'success' do
      it 'renders template update' do
        xhr :put, :update, { id: @script.id,
                             script: { name: 'Changed' }}
        @script.reload
        expect(@script.name).to eq 'Changed'
      ...
      it 'redirects to index' do
        put :update, { id: @script.id,
                       script: { name: 'Changed' }}
        expect(response).to redirect_to('/environment/scripted_resources')
      end
    end
  ...
  context '#create' do
    ...
    context 'fails' do
      ...
      it 'returns paginated records and show validation errors' do
        ...
        @scripts[0..2].each { |el| expect(scripts).to include(el) }
        expect(scripts).to_not include(@scripts[3])
        expect(response).to render_template('misc/error_messages_for')
      end

      it 'renders template new' do
        post :create, @params
        expect(response).to render_template('scripted_resources/detail_new')
      end
    end
  end
end
