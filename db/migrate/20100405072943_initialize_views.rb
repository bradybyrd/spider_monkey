...
################################################################################
 ...
 class InitializeViews < ActiveRecord::Migration
...
SQL
  ...
  def self.down
    if PorttfolioSupport
    #### Commented SmartPortfolio related code. It can be deleted after BRPM 2.5...
    execute <<-SQL
CREATE OR REPLACE VIEW containers_view (id, projected_cost, name, type, group_name, manager_name) AS
SELECT containers.id, COALESCE(SUM(budget_line_items.projected_cost), 0), containers.name,
containers.container_type, groups.name, users.first_name || ' ' || users.last_name FROM containers
INNER JOIN users ON users.id = containers.manager_id
INNER JOIN groups ON groups.id = containers.group_id
LEFT OUTER JOIN parent_activities ON parent_activities.container_id = containers.id
LEFT OUTER JOIN budget_line_items ON budget_line_items.activity_id = parent_activities.activity_id
GROUP BY containers.id, containers.name, containers.container_type, groups.name, users.first_name || ' ' || users.last_name
SQL

    execute <<-SQL
CREATE OR REPLACE VIEW activities_view (id, projected_cost, name, type, group_name, budget_category, manager_name, container_id) AS
SELECT activities.id, COALESCE(SUM(budget_line_items.projected_cost), 0), activities.name,
activity_categories.name, groups.name, activities.budget_category,
users.first_name || ' ' || users.last_name, parent_activities.container_id FROM activities
...
LEFT OUTER JOIN groups ON groups.id = activities.leading_group_id
LEFT OUTER JOIN budget_line_items ON budget_line_items.activity_id = activities.id
GROUP BY activities.id, activities.name,
activity_categories.name, groups.name, activities.budget_category,
users.first_name || ' ' || users.last_name, parent_activities.container_id
SQL
  
    execute <<-SQL
CREATE OR REPLACE VIEW financials_view 
(container_id, container_name, container_projected_cost, container_type,
container_group_name, container_manager_name, activity_id, activity_name, activity_projected_cost,
activity_type, activity_group_name, activity_budget_category, activity_manager_name) AS

SELECT containers_view.id, containers_view.name, containers_view.projected_cost, containers_view.type,
containers_view.group_name, containers_view.manager_name, activities_view.id, activities_view.name, 
activities_view.projected_cost, activities_view.type, activities_view.group_name,
activities_view.budget_category, activities_view.manager_name FROM containers_view

INNER JOIN activities_view ON activities_view.container_id = containers_view.id
GROUP BY containers_view.id, containers_view.name, containers_view.projected_cost, containers_view.type,
containers_view.group_name, containers_view.manager_name, activities_view.id, activities_view.name, 
activities_view.projected_cost, activities_view.type, activities_view.group_name,
activities_view.budget_category, activities_view.manager_name
SQL
